			 ...
			 <form method="post">					
				<div class="row">
					<div class="form-group col-md-4 col-md-offset-0">
						<label for="arena">Nama Futsal</label>
						<select name="arena" class="form-control" onchange="loadData(this.value, '1')">
							<option value="0">.: Select :.</option>
							<?php for($i=0;$i<count($d_arena);$i++){ ?>
								<option value="<?= $d_arena[$i]['id_arena'] ?>">
									<?= $d_arena[$i]['nama'] ?>
								</option>
							<?php } ?>
						</select>
					...
					<div class="form-group col-md-6">
						<br/>
						<a href="?menu=booking&action=history" type="button" class="btn btn-primary">History</a>
					</div>
				</div>
				<div class="row">
					<div class="form-group col-md-4 col-md-offset-0">
						<label for="lapangan">Lapangan</label>
						<select name="lapangan" class="form-control" id="lapangan" onchange="loadData(this.value, '2')">
							<option value="0">.: Select :.</option>
							<?php for($i=0;$i<count($d_lapangan);$i++){ ?>
								<option value="<?= $d_lapangan[$i]['id_lapangan'] ?>">
									<?= $d_arena[$i]['nama'] ?>
								</option>
							<?php } ?>
						</select>
				...
				<div class="row">
					<div class="form-group col-md-3">									
						...
						<input class="form-control" type="" id="tgl" name="tgl" onchange="loadData(this.value, '2')" required />
					</div>
					<div class="form-group col-md-2">
						<label for="jam" id="jam_div">Jam</label>
						<input class="form-control" type="" id="jam" name="jam" onchange="loadData(this.value, '3')" required />							
					</div>
				</div>	
				<div class="row">				
					<div class="form-group col-md-2">
				...
				<div class="row">
				    <div class="col-md-2">
				        <input type="submit" name="booking" value="SUBMIT" style="width:100%">
				    </div>
				        <div class="col-md-6 alert alert-info">Minimal DP yang harus di bayar adalah 30% dari total pembayaran dan dibayar paling lambat 30 sebelum jam main, serta <b style="color:#ce7171">keterlambatan lebih dari 15 menit akan di anggap batal</b> 
				    </div>
				</div>
			</form>		
		 </div>
...
<script>
	...
	function loadData(val, type){	
		...
		if(status == 1){
			...
			xmlhttp.open("GET","index.php?menu=" + link + "&action=loadData&id=" + id + "&tgl=" + tgl + "&type=" + type,true);
			xmlhttp.send(null);   
		}		  
	}
